                        ...
                        list<word *>::iterator &from, bool inclusive)
{
    ...
    //cout << "DEBUG: FILTER: 1" << endl;

//    for (word * w : oldList)
  //      cout << "DEBUG: oldList: " << w->s << endl;

    auto iter = from;
    if (!inclusive)
        iter++;

   // cout << "DEBUG: FILTER: 2" << endl;
    for (; iter != oldList.end(); iter++) {
        /* iter points to a pointer to a word */

    //cout << "DEBUG: FILTER: 2.25:" << (**iter).s << endl;
        if (containsAllLetters(remaining, (**iter).count)) {


                newList.emplace_back(*iter);
    //cout << "DEBUG: FILTER: 2.5: " << (**iter).s << endl;
        }

      //      if (iter == oldList.end())
        //    cout << "DEBUG: FILTER: 2.325" << endl;

    }

    //cout << "DEBUG: FILTER: 3" << endl;
    return newList;
}
...
void findSolutionsRecur(solution partialSolution, list<word *> wordList)
{
    //cout << "DEBUG: partial solution: " << partialSolution.s << endl;
